...
def display_scatterplots(df: pd.DataFrame, tvl_df: pd.DataFrame, alpha: float) -> None:
    ...
    if selected_metric == "TVL":
        prv_df = df.copy()
        st.write(prv_df.columns)

        df = tvl_df
        df.rename(columns={"project":"Project", "percent_change":"Percent Change", "p_value": "P Value"}, inplace=True)
        ...
        df = pd.merge(df, prv_df[["Project", "Grant Amount"]], on="project", left="left")
    else:
