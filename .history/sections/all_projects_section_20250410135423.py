...
def display_scatterplots(df: pd.DataFrame, tvl_df: pd.DataFrame, alpha: float) -> None:
    ...
    if selected_metric == "TVL":
        st.write(tvl_df.columns)
        st.dataframe(tvl_df)
        df = tvl_df
        df.rename(columns={"percent_change":"Percent Change", "p_value": "P Value"}, inplace=True)
        df["Change"] = "increase" if df["P Value"] <= alpha and df["Percent Change"] > 0 else "decrease" if df["P Value"] <= alpha and df["Percent Change"] < 0 else "no change"
    else:
