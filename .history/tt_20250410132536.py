...
for project_name, project in projects.items():
    ...
    if 'daily_transactions' in datasets.keys():
        ...
        # Find first non-zero day based on either column
        nonzero_mask = (df['num_transactions'] != 0) | (df['transaction_volume'] != 0)
        if nonzero_mask.any():
            first_valid_date = df.loc[nonzero_mask, 'transaction_date'].min()
            df = df[df['transaction_date'] >= first_valid_date].reset_index(drop=True)

        print(f"{project_name} trimmed to start from first non-zero day: {first_valid_date.date()}")
